...
<body>
	...
	<div class="slide-box"
		...
		<a href="#"><img src="img/slide1.jpg" alt="slide"></a> <a
			href="#"><img src="img/slide2.jpg" alt="slide"></a> <a href="#"><img
			src="img/slide3.jpg" alt="slide"></a> <a href="#"><img
			src="img/slide4.jpg" alt="slide"></a>
	</div>
	...
	<div style="margin: 0 15% 0 15%; padding: 40px 0 40px 0;">
		<form action="index.do" method="post">
			...
			</select> <select name="min" onchange="cus()">
			...
			</select> <input type="submit" value="검색"> <br>
			<br>

			<c:if test="${list==null }"><jsp:forward page="index.do"></jsp:forward>
			</c:if>

			<table width="70%" border="0" cellpadding="4" cellspacing="4">
				<tr>
					<c:forEach items="${list }" var="list">
						<td class="hover" width="17%" height="250px" bgColor="#DDDDDD"><a
							href="productDetail.do?product_id=${list.product_id }"><br>${list.product_name }<br>
							<br>
							<img src="img/product/${list.main_img }" class="item_img"><br>${list.category }<br>${list.quality }<br>\
								${list.price }</a></td>
			...
			<center>
				<c:forEach var="pageNum" begin="1" end="${pageMax }">
					<a
						href="index.do?pageNum=${pageNum }&category=${category}&quality=${quality}&max=${max }&min=${min }">${pageNum}</a>
				</c:forEach>
			</center>

			<!--       각 상품DIV의 크기 기준을 hr의 길이로 설정하기 위해 설정. -->
			<hr id="hr" style="visibility: hidden;">

		</form>

		<!--    	검색조건을 유지하기 위해, 요청이 일어난 곳의 정보를 Hidden타입으로 숨겨둠. -->
		<input type="hidden" id="category2" value="${category }"> <input
			type="hidden" id="max2" value="${max }"> <input type="hidden"
			id="min2" value="${min }"> <input type="hidden" id="quality2"
			value="${quality }">
	</div>
</body>
</html>